...
// PURPOSE: Display motorcycle patrol reports
// =====================================================
...
// Build query
$query = "
    SELECT p.patrol_id, p.specific_location, p.patrol_date, p.patrol_time, 
           p.personnel_count, p.vehicle_number, p.accomplishment_description, p.status, p.submitted_at,
    ...
    WHERE p.patrol_type = 'Motorcycle Patrol'
";
...
$result = $conn->query("SELECT SUM(personnel_count) as total FROM patrol_activities WHERE patrol_type = 'Motorcycle Patrol'");
$stats['personnel'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT COUNT(DISTINCT vehicle_number) as total FROM patrol_activities WHERE patrol_type = 'Motorcycle Patrol' AND vehicle_number IS NOT NULL");
...
<body class="flex bg-[#0a3d62]">
        ...
        <ul class="space-y-1">
            ...
            <li class="dropdown active">
                ...
                <ul class="pl-8 mt-1 space-y-1 dropdown-content" style="display: block;">
                    ...
                    <li class="py-2 px-3 text-sm bg-[#0a3d62] rounded"><a href="motorpatrol.php" class="text-white no-underline block">Motorcycle Patrol</a></li>
                </ul>
            </li>
    ...
    <div class="flex-1 p-8 bg-[#eef2f6] overflow-y-auto h-screen">
        <h2 class="text-2xl font-bold text-[#08324f] mb-6">Motorcycle Patrol Reports</h2>

        <!-- Summary Cards -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white p-4 rounded-lg shadow-sm">
                <p class="text-gray-600 text-sm">Total Motor Patrols</p>
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm">
                <p class="text-gray-600 text-sm">Motor Units Used</p>
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-[#08324f] text-white">
                        ...
                        <th class="p-3 text-center">Personnel</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="9" class="p-6 text-center text-gray-500">No motorcycle patrol reports found</td>
                    </tr>
                    <?php else: ?>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                        ...
                        <td class="p-3"><?php echo date('M d, Y', strtotime($row['patrol_date'])); ?></td>
                        <td class="p-3"><?php echo date('h:i A', strtotime($row['patrol_time'])); ?></td>
                        <td class="p-3 text-center"><?php echo $row['personnel_count']; ?></td>
                        <td class="p-3"><?php echo $row['vehicle_number'] ?? 'N/A'; ?></td>
                        <td class="p-3">
                        ...
                        <td class="p-3">
                            <a href="view_report.php?type=patrol&id=<?php echo $row['patrol_id']; ?>" class="bg-[#0a3d62] text-white px-3 py-1 rounded text-xs hover:bg-[#08324f]">View</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                    <?php endif; ?>
