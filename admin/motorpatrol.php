<?php
// =====================================================
// FILE: admin/motorpatrol.php
// PURPOSE: Display motorcycle patrol reports (auto-approved only)
...
$query = "
    ...
    WHERE p.patrol_type = 'Motorcycle Patrol' AND p.status = 'approved'
";
...
if (!empty($search)) {
    ...
    $query .= " AND (u.first_name LIKE '%$search%' OR u.last_name LIKE '%$search%' 
                ...
                OR p.specific_location LIKE '%$search%' OR p.vehicle_number LIKE '%$search%')";
}
...
$result = $conn->query("SELECT SUM(personnel_count) as total FROM patrol_activities WHERE patrol_type = 'Motorcycle Patrol' AND status = 'approved'");
$stats['personnel'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT COUNT(DISTINCT vehicle_number) as total FROM patrol_activities WHERE patrol_type = 'Motorcycle Patrol' AND vehicle_number IS NOT NULL AND status = 'approved'");
$stats['units'] = $result->fetch_assoc()['total'] ?? 0;

// Violations totals for motorcycle patrol
...
FROM patrol_activities WHERE patrol_type = 'Motorcycle Patrol' AND status = 'approved'");
...
<head>
    ...
    <title>PNP | Motorcycle Patrol Reports</title>
        ...
        <ul class="space-y-1 px-3 pb-5">
            ...
            <li class="dropdown active">
                ...
                <ul class="dropdown-content pl-4 ml-4 space-y-1 border-l border-[#1e4a6a]" style="display: block;">
                    ...
                    <li class="bg-[#1e4a6a] rounded-lg"><a href="motorpatrol.php" class="block p-2 text-sm text-yellow-400">Motor Patrol</a></li>
                </ul>
            </li>
    ...
    <div class="flex-1 p-4 md:p-6 lg:p-8 bg-[#eef2f6] overflow-y-auto min-h-screen main-content-mobile">
        ...
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 border-l-4 border-yellow-400 flex justify-between items-center">
            <div>
                <h2 class="text-xl md:text-2xl font-bold text-[#08324f] flex items-center gap-2">
                    ...
                    Motorcycle Patrol Reports
                </h2>
                <p class="text-sm text-gray-600 mt-1">All motorcycle patrol reports are automatically approved</p>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            <div class="stat-card bg-white p-4 rounded-lg shadow-md border-l-4 border-blue-500">
                <p class="text-xs text-gray-500">Total Motor Patrols</p>
            ...
            <div class="stat-card bg-white p-4 rounded-lg shadow-md border-l-4 border-purple-500">
                <p class="text-xs text-gray-500">Motor Units Used</p>
                <p class="text-2xl font-bold text-[#08324f]"><?php echo $stats['units']; ?></p>
            </div>
            ...
            <form method="GET" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex-1 min-w-[250px]">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" 
                           placeholder="Officer, badge, unit, barangay, location..." 
                ...
                <div class="flex gap-2">
                    ...
                    <a href="motorpatrol.php" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm flex items-center gap-1">
        ...
        <div class="flex justify-between items-center mb-3 text-sm text-gray-600">
            <span><i class="fas fa-list mr-1"></i> Showing <strong><?php echo $patrols->num_rows; ?></strong> motorcycle patrol reports</span>
        </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="9" class="p-8 text-center text-gray-500">
                                <i class="fas fa-motorcycle text-4xl mb-3 text-gray-300"></i>
                                <p>No motorcycle patrol reports found</p>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td>
                                ...
                                <span class="unit-badge">
                                    <i class="fas fa-motorcycle mr-1"></i> <?php echo $row['vehicle_number']; ?>
                                </span>
                                <?php else: ?>
        ...
        <div class="mt-4 text-xs text-gray-500 text-center">
            <i class="fas fa-check-circle text-green-500 mr-1"></i> All motorcycle patrol reports are auto-approved
        </div>
    </div>
