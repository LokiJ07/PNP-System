<?php
// =====================================================
// FILE: admin/oplanbakal.php
// PURPOSE: Display Oplan Bakal reports (auto-approved only)
...
$query = "
    ...
    WHERE o.oplan_type = 'Oplan Bakal' AND o.status = 'approved'
";
...
$result = $conn->query("SELECT SUM(personnel_count) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal' AND status = 'approved'");
$stats['personnel'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(operations_count) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal' AND status = 'approved'");
...
$result = $conn->query("SELECT SUM(arrests_made) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal' AND status = 'approved'");
$stats['arrests'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(house_visitations) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal' AND status = 'approved'");
        ...
        <ul class="space-y-1 px-3 pb-5">
            ...
            <li class="dropdown active">
                ...
                <ul class="dropdown-content pl-4 ml-4 space-y-1 border-l border-[#1e4a6a]" style="display: block;">
                    <li class="bg-[#1e4a6a] rounded-lg"><a href="oplanbakal.php" class="block p-2 text-sm text-yellow-400">Oplan Bakal</a></li>
    ...
    <div class="flex-1 p-4 md:p-6 lg:p-8 bg-[#eef2f6] overflow-y-auto min-h-screen main-content-mobile">
        ...
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 border-l-4 border-yellow-400 flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">All Oplan Bakal reports are automatically approved</p>
            </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 mb-6">
            ...
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-red-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['firearms_seized']; ?></p>
            </div>
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-orange-500">
                <p class="text-xs text-gray-500">CRS</p>
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['firearms_crs']; ?></p>
            </div>
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-yellow-500">
                <p class="text-xs text-gray-500">FAS Dep</p>
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['fas_deposit']; ?></p>
            </div>
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-indigo-500">
                <p class="text-xs text-gray-500">Renew FAS</p>
            ...
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-pink-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['arrests']; ?></p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
            ...
            <div class="bg-white p-4 rounded-lg shadow-md">
                <h3 class="text-sm font-semibold text-[#08324f] mb-3 flex items-center gap-2">
                    ...
                    Average Firearms per Operation
                </h3>
                <p class="text-2xl font-bold text-[#08324f]">
                    <?php echo $stats['total'] > 0 ? round($stats['firearms_seized'] / $stats['total'], 2) : 0; ?>
                </p>
            </div>
        </div>
        ...
        <div class="filter-card">
            <form method="GET" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="oplanbakal.php" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm flex items-center gap-1">
        ...
        <div class="flex justify-between items-center mb-3 text-sm text-gray-600">
            <span><i class="fas fa-list mr-1"></i> Showing <strong><?php echo $oplans->num_rows; ?></strong> Oplan Bakal reports</span>
        </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Firearms</th>
                            <th class="text-center">CRS</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="13" class="p-8 text-center text-gray-500">
                                ...
                                <p>No Oplan Bakal reports found</p>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="text-center">
                                <?php if ($row['firearms_seized'] > 0): ?>
                                <span class="firearm-badge">
                                    <i class="fas fa-gun mr-1"></i> <?php echo $row['firearms_seized']; ?>
                                </span>
                                <?php else: ?>
                                0
                                <?php endif; ?>
                            </td>
                            <td class="text-center"><?php echo $row['firearms_crs']; ?></td>
                            <td class="text-center"><?php echo $row['fas_deposit']; ?></td>
                            <td class="text-center"><?php echo $row['renewed_fas']; ?></td>
        ...
        <div class="mt-4 text-xs text-gray-500 flex flex-wrap justify-between">
            <span><i class="fas fa-check-circle text-green-500 mr-1"></i> All Oplan Bakal reports are auto-approved</span>
            <span><i class="fas fa-gun text-red-600 mr-1"></i> Firearms Seized</span>
        </div>
    </div>
