...
// INCLUDES: Violations, Disposition, Oplan Bakal/Sita stats
// =====================================================
...
// ===== NEW: CHECKPOINT DISPOSITION STATISTICS =====
$result = $conn->query("SELECT SUM(fixed_count) as total FROM checkpoint_activities");
$stats['checkpoint_fixed'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(fined_count) as total FROM checkpoint_activities");
$stats['checkpoint_fined'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(warned_count) as total FROM checkpoint_activities");
$stats['checkpoint_warned'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(charged_count) as total FROM checkpoint_activities");
$stats['checkpoint_charged'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(community_service) as total FROM checkpoint_activities");
...
$result = $conn->query("SELECT SUM(firearms_crs) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal'");
$stats['firearms_crs'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(fas_deposit) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal'");
$stats['fas_deposit'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(renewed_fas) as total FROM oplan_activities WHERE oplan_type = 'Oplan Bakal'");
...
$result = $conn->query("SELECT SUM(kontra_boga) as total FROM oplan_activities WHERE oplan_type = 'Oplan Sita'");
$stats['kontra_boga'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(anti_vaping) as total FROM oplan_activities WHERE oplan_type = 'Oplan Sita'");
$stats['anti_vaping'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(house_visitations) as total FROM oplan_activities WHERE oplan_type = 'Oplan Sita'");
...
$result = $conn->query("SELECT SUM(house_visitations) as total FROM oplan_activities");
$stats['oplan_house_visits'] = $result->fetch_assoc()['total'] ?? 0;

// ===== NEW: OPLAN DISPOSITION STATISTICS =====
$result = $conn->query("SELECT SUM(fixed_count) as total FROM oplan_activities");
$stats['oplan_fixed'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(fined_count) as total FROM oplan_activities");
$stats['oplan_fined'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(warned_count) as total FROM oplan_activities");
$stats['oplan_warned'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("SELECT SUM(charged_count) as total FROM oplan_activities");
$stats['oplan_charged'] = $result->fetch_assoc()['total'] ?? 0;

// ===== VIOLATIONS STATISTICS (from all tables) =====
$result = $conn->query("
    SELECT 
        (SELECT SUM(drinking_violations) FROM patrol_activities) +
        (SELECT SUM(drinking_violations) FROM checkpoint_activities) +
        (SELECT SUM(drinking_violations) FROM oplan_activities) as total
");
$stats['drinking_violations'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("
    SELECT 
        (SELECT SUM(smoking_violations) FROM patrol_activities) +
        (SELECT SUM(smoking_violations) FROM checkpoint_activities) +
        (SELECT SUM(smoking_violations) FROM oplan_activities) as total
");
$stats['smoking_violations'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("
    SELECT 
        (SELECT SUM(halfnaked_violations) FROM patrol_activities) +
        (SELECT SUM(halfnaked_violations) FROM checkpoint_activities) +
        (SELECT SUM(halfnaked_violations) FROM oplan_activities) as total
");
$stats['halfnaked_violations'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("
    SELECT 
        (SELECT SUM(curfew_violations) FROM patrol_activities) +
        (SELECT SUM(curfew_violations) FROM checkpoint_activities) +
        (SELECT SUM(curfew_violations) FROM oplan_activities) as total
");
$stats['curfew_violations'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("
    SELECT 
        (SELECT SUM(vandalism_violations) FROM patrol_activities) +
        (SELECT SUM(vandalism_violations) FROM checkpoint_activities) +
        (SELECT SUM(vandalism_violations) FROM oplan_activities) as total
");
$stats['vandalism_violations'] = $result->fetch_assoc()['total'] ?? 0;

$result = $conn->query("
    SELECT 
        (SELECT SUM(other_violations) FROM patrol_activities) +
        (SELECT SUM(other_violations) FROM checkpoint_activities) +
        (SELECT SUM(other_violations) FROM oplan_activities) as total
");
...
<head>
    ...
    <style>
        ...
        /* PNP Colors */
        .pnp-navy { background-color: #08324f; }
        ...
        .gradient-violations {
            background: linear-gradient(135deg, #856404 0%, #b38b00 100%);
        }
    </style>
</head>
<body class="flex flex-col md:flex-row bg-[#08324f] min-h-screen">
    ...
    <div id="sidebar" class="w-full md:w-[260px] bg-[#08324f] text-white h-screen overflow-y-auto sidebar-scroll sidebar-mobile fixed top-0 left-[-100%] md:left-0 md:sticky z-50 transition-all duration-300 ease-in-out">
        ...
        <div class="bg-[#1e4a6a] mx-4 my-4 p-4 rounded-lg text-center shadow-lg">
            <div class="w-16 h-16 bg-[#ffc107] rounded-full mx-auto mb-3 flex items-center justify-center text-[#08324f] text-2xl font-bold">
        ...
        <ul class="space-y-1 px-3 pb-5">
            <li>
                <a href="admin_dashboard.php" class="flex items-center gap-3 p-3 rounded-lg bg-[#1e4a6a] border-l-4 border-yellow-400 hover:bg-[#1e4a6a] transition">
            ...
            <li class="dropdown">
                ...
                <ul class="dropdown-content pl-4 ml-4 space-y-1 border-l border-[#1e4a6a]">
                    ...
                    <li><a href="motorpatrol.php" class="block p-2 text-sm hover:bg-[#1e4a6a] rounded-lg transition">Motorcycle Patrol</a></li>
                </ul>
            </li>
            <li class="dropdown">
                <div class="flex items-center justify-between p-3 rounded-lg hover:bg-[#1e4a6a] cursor-pointer transition" onclick="toggleDropdown(this)">
    ...
    <div class="flex-1 p-4 md:p-6 lg:p-8 bg-[#eef2f6] overflow-y-auto min-h-screen main-content-mobile">
        ...
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 border-l-4 border-yellow-400 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            <div>
                <h2 class="text-xl md:text-2xl font-bold text-[#08324f]">Dashboard Overview</h2>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
            <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-[#08324f]">
            ...
            <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-[#c41e3a]">
                <div class="flex justify-between items-start">
                    <div>
        ...
        <!-- VIOLATIONS STATISTICS - NEW SECTION -->
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-[#08324f] mb-3 flex items-center gap-2">
                <i class="fas fa-gavel text-yellow-500"></i> Ordinance Violations Summary
            </h3>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                <div class="bg-white p-3 rounded-lg shadow-sm border-b-2 border-blue-500">
        ...
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-[#08324f] mb-3 flex items-center gap-2">
                <i class="fas fa-walking text-yellow-500"></i> Patrol Statistics
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="stat-card gradient-patrol p-4 rounded-lg shadow-lg text-white">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-xs bg-white/20 px-2 py-1 rounded-full"><?php echo $stats['foot_patrols']; ?> total</span>
                    </div>
                ...
                <div class="stat-card gradient-patrol p-4 rounded-lg shadow-lg text-white">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-xs bg-white/20 px-2 py-1 rounded-full"><?php echo $stats['mobile_patrols']; ?> total</span>
                    </div>
                ...
                <div class="stat-card gradient-patrol p-4 rounded-lg shadow-lg text-white">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-xs bg-white/20 px-2 py-1 rounded-full"><?php echo $stats['motor_patrols']; ?> total</span>
                    </div>
        ...
        <!-- Checkpoint & Oplan Stats with Disposition -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            <!-- Checkpoint Stats -->
            <div>
                <h3 class="text-lg font-semibold text-[#08324f] mb-3 flex items-center gap-2">
                    <i class="fas fa-map-marker-alt text-yellow-500"></i> Checkpoint Statistics
                </h3>
                <div class="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div class="stat-card gradient-checkpoint p-4 rounded-lg shadow-lg text-white">
                        ...
                        <p class="text-sm opacity-90">Arrests Made</p>
                    </div>
                </div>
                
                <!-- Checkpoint Disposition -->
                <div class="bg-white p-3 rounded-lg shadow-sm">
                    <h4 class="text-sm font-semibold text-[#08324f] mb-2">Disposition Summary</h4>
            ...
            <div>
                <h3 class="text-lg font-semibold text-[#08324f] mb-3 flex items-center gap-2">
                    <i class="fas fa-shield-alt text-yellow-500"></i> Oplan Statistics
                </h3>
                
                <!-- Oplan Bakal -->
                <div class="mb-3">
                    <h4 class="text-sm font-semibold text-[#08324f] mb-2">Oplan Bakal (Firearms)</h4>
                    <div class="grid grid-cols-2 gap-3">
                        <div class="stat-card gradient-patrol p-3 rounded-lg shadow-lg text-white">
                            <p class="text-xs opacity-90">Operations</p>
                            <p class="text-xl font-bold"><?php echo $stats['oplan_bakal']; ?></p>
                        </div>
                        <div class="stat-card gradient-patrol p-3 rounded-lg shadow-lg text-white">
                            <p class="text-xs opacity-90">Firearms Seized</p>
                            <p class="text-xl font-bold"><?php echo $stats['firearms_seized']; ?></p>
                        </div>
                        <div class="stat-card gradient-patrol p-3 rounded-lg shadow-lg text-white">
                            <p class="text-xs opacity-90">FAS Deposits</p>
                            <p class="text-xl font-bold"><?php echo $stats['fas_deposit']; ?></p>
                        </div>
                        <div class="stat-card gradient-patrol p-3 rounded-lg shadow-lg text-white">
                            <p class="text-xs opacity-90">Renewed FAS</p>
                            <p class="text-xl font-bold"><?php echo $stats['renewed_fas']; ?></p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Oplan Sita -->
                <div class="mb-3">
                    <h4 class="text-sm font-semibold text-[#08324f] mb-2">Oplan Sita (Ordinances)</h4>
                    <div class="grid grid-cols-3 gap-2">
                        <div class="bg-white p-2 rounded-lg shadow-sm">
                            <p class="text-xs text-gray-500">Operations</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['oplan_sita']; ?></p>
                        </div>
                        <div class="bg-white p-2 rounded-lg shadow-sm">
                            <p class="text-xs text-gray-500">Kontra Boga</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['kontra_boga']; ?></p>
                        </div>
                        <div class="bg-white p-2 rounded-lg shadow-sm">
                            <p class="text-xs text-gray-500">Anti-Vaping</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['anti_vaping']; ?></p>
                        </div>
                        <div class="bg-white p-2 rounded-lg shadow-sm">
                            <p class="text-xs text-gray-500">Contraband</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['contraband']; ?> kg</p>
                        </div>
                        <div class="bg-white p-2 rounded-lg shadow-sm">
                            <p class="text-xs text-gray-500">House Visits</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['sita_house_visits']; ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-2">
                    <div class="bg-white p-3 rounded-lg shadow-sm">
                        ...
                        <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['oplan_arrests']; ?></p>
                    </div>
                    <div class="bg-white p-3 rounded-lg shadow-sm">
                ...
                <!-- Oplan Disposition -->
                <div class="bg-white p-3 rounded-lg shadow-sm mt-3">
                    <h4 class="text-sm font-semibold text-[#08324f] mb-2">Oplan Disposition</h4>
                    <div class="grid grid-cols-4 gap-2 text-center">
                        <div>
                            <p class="text-xs text-gray-500">Fixed</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['oplan_fixed']; ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Fined</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['oplan_fined']; ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Warned</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['oplan_warned']; ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500">Charged</p>
                            <p class="font-bold text-[#08324f]"><?php echo $stats['oplan_charged']; ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <?php endwhile; ?>
                <?php if ($top_officers->num_rows == 0): ?>
    ...
    <script>
        ...
        function checkNotifications() {
            // Simplified - you can implement actual notification logic
