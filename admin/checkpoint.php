...
// PURPOSE: Display all checkpoint reports (auto-approved only)
// REMOVED: Pending and Rejected status
// =====================================================
...
if ($barangay_id > 0) {
    $query .= " AND c.barangay_id = $barangay_id";
}

if (!empty($from_date)) {
    $query .= " AND DATE(c.checkpoint_date) >= '" . $conn->real_escape_string($from_date) . "'";
}

if (!empty($to_date)) {
    $query .= " AND DATE(c.checkpoint_date) <= '" . $conn->real_escape_string($to_date) . "'";
}
...
if (!empty($search)) {
    ...
    $query .= " AND (u.first_name LIKE '%$search%' OR u.last_name LIKE '%$search%' 
                ...
                OR c.specific_location LIKE '%$search%')";
}

$query .= " ORDER BY c.checkpoint_date DESC, c.checkpoint_time DESC";

$checkpoints = $conn->query($query);

// Get statistics (all approved)
$stats = [];
...
FROM checkpoint_activities WHERE status = 'approved'");
$ops = $result->fetch_assoc();
$stats['border_ops'] = $ops['border_ops'];
$stats['mobile_ops'] = $ops['mobile_ops'];
$stats['tct_ovr'] = $ops['tct_ovr'];
$stats['arrests'] = $ops['arrests'];
...
<body class="flex flex-col md:flex-row bg-[#08324f] min-h-screen">
    ...
    <div id="sidebar" class="w-full md:w-[260px] bg-[#08324f] text-white h-screen overflow-y-auto sidebar-scroll sidebar-mobile fixed top-0 left-[-100%] md:left-0 md:sticky z-50 transition-all duration-300 ease-in-out">
        ...
        <ul class="space-y-1 px-3 pb-5">
            ...
            <li class="bg-[#1e4a6a] rounded-lg"><a href="checkpoint.php" class="flex items-center gap-3 p-3"><i class="fas fa-map-marker-alt w-5 text-yellow-400"></i> Checkpoint</a></li>
    ...
    <div class="flex-1 p-4 md:p-6 lg:p-8 bg-[#eef2f6] overflow-y-auto min-h-screen main-content-mobile">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
            ...
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-indigo-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['border_ops']; ?></p>
            </div>
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-purple-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['mobile_ops']; ?></p>
            </div>
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-orange-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['tct_ovr']; ?></p>
            </div>
            ...
            <div class="stat-card bg-white p-3 rounded-lg shadow-md border-l-4 border-green-500">
                ...
                <p class="text-xl font-bold text-[#08324f]"><?php echo $stats['total_violations']; ?></p>
            </div>
        </div>
        ...
        <div class="filter-card">
            <form method="GET" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="checkpoint.php" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm flex items-center gap-1">
        ...
        <div class="flex justify-between items-center mb-3 text-sm text-gray-600">
            <span><i class="fas fa-list mr-1"></i> Showing <strong><?php echo $checkpoints->num_rows; ?></strong> checkpoint reports</span>
        </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">TCT/OVR</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="11" class="p-8 text-center text-gray-500">
                                ...
                                <p>No checkpoint reports found</p>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap"><?php echo date('M d, Y', strtotime($row['checkpoint_date'])); ?></td>
                            <td class="whitespace-nowrap"><?php echo date('h:i A', strtotime($row['checkpoint_time'])); ?></td>
                            <td class="text-center font-bold"><?php echo $row['border_control_ops']; ?></td>
                            <td class="text-center font-bold"><?php echo $row['mobile_checkpoint_ops']; ?></td>
                            <td class="text-center font-bold"><?php echo $row['tct_ovr_accomplishment']; ?></td>
                            <td class="text-center font-bold text-red-600"><?php echo $row['arrested_accomplishment']; ?></td>
                            <td>
                                <a href="view_report.php?type=checkpoint&id=<?php echo $row['checkpoint_id']; ?>" 
        ...
        <div class="mt-4 text-xs text-gray-500 flex justify-between">
            <span><i class="fas fa-check-circle text-green-500 mr-1"></i> All reports are auto-approved</span>
            <span><i class="fas fa-info-circle text-gray-400 mr-1"></i> Click View for complete details including violations</span>
        </div>
    </div>
